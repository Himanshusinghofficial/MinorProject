...
cv2.setMouseCallback("Video Processing", get_mouse_points)
num_mouse_points = 0
...
while cap.isOpened():
    ...
    if not ret:
        print("Finished final Result:",str(int(total_six_feet_violations)),"violations")
        break
    ...
    print("frame number", frame_num, ":",str(int(total_six_feet_violations)) ,"violations")

    # draw polygon of ROI
    pts = np.array(
        [four_points[0], four_points[1], four_points[3], four_points[2]], np.int32
    )
    cv2.polylines(frame, [pts], True, (255, 0, 0), thickness=4)
    ...
    # text = "Stay-at-home Index: " + str(np.round(150 * sh_index, 1)) + "%"
    # pedestrian_detect, last_h = put_text(pedestrian_detect, text, text_offset_y=last_h)
